...
class GenealogyViewer:
    ...
    def open_recovery_wizard(self):
        ...
        info = tk.LabelFrame(right, text="Семейный контекст")
        info.pack(fill="x", pady=(0, 10))
        self._recovery_context = tk.Text(info, height=12, wrap="word")
        self._recovery_context.pack(fill="both", expand=True, padx=6, pady=6)
        self._recovery_context.configure(state="disabled")

        form = tk.LabelFrame(right, text="Заполните существующую карточку")
        ...
        for row, (key, label) in enumerate(fields):
            tk.Label(form, text=label + ":", width=18, anchor="e").grid(row=row, column=0, sticky="e", padx=5, pady=3)
            var = tk.StringVar()
            self._recovery_vars[key] = var
            tk.Entry(form, textvariable=var, width=58).grid(row=row, column=1, sticky="ew", padx=5, pady=3)
        tk.Label(form, text="Примечание:", width=18, anchor="ne").grid(row=len(fields), column=0, sticky="ne", padx=5, pady=3)
        self._recovery_note = tk.Text(form, height=6, wrap="word")
        self._recovery_note.grid(row=len(fields), column=1, sticky="nsew", padx=5, pady=3)
        ...
        self._recovery_list.selection_set(0)
        self._load_recovery_record(0)

    def _close_recovery_wizard(self):
        if self._recovery_window is not None:
            try:
    ...
    def _load_recovery_record(self, index):
        ...
        values = {
            "first_name": "", "last_name": "", "sex": record.sex,
            "birth_date": record.birth_date, "birth_place": record.birth_place,
            "death_date": record.death_date, "death_place": record.death_place,
            "occupation": record.occupation,
        }
        for key, var in self._recovery_vars.items():
            var.set(values.get(key, ""))
        self._recovery_note.delete("1.0", "end")
        self._recovery_note.insert("1.0", record.note)
        context = [
            f"Родители: {', '.join(record.parents) if record.parents else 'нет данных'}",
            f"Супруги/партнёры: {', '.join(record.partners) if record.partners else 'нет данных'}",
            f"Дети: {', '.join(record.children) if record.children else 'нет данных'}",
            f"Событий: {record.event_count}",
        ]
        self._recovery_context.configure(state="normal")
        self._recovery_context.delete("1.0", "end")
        self._recovery_context.insert("1.0", "\n".join(context))
        self._recovery_context.configure(state="disabled")

    def _save_recovery_person(self):
        ...
        data = {key: var.get() for key, var in self._recovery_vars.items()}
        data["note"] = self._recovery_note.get("1.0", "end").strip()
        try:
        ...
        if not self._recovery_records:
            messagebox.showinfo("Мастер восстановления", "Все найденные пустые карточки обработаны.")
    ...
    def _skip_recovery_person(self):
        if not self._recovery_records:
            return
    ...
    def _open_recovery_person_card(self):
        if self._recovery_index < 0 or self._recovery_index >= len(self._recovery_records):
            return
