...
class GenealogyViewer:
    ...
    def _create_widgets(self):
        ...
        help_menu.add_command(label="Центр релиза", command=self.open_release_center)
        help_menu.add_command(label="Готовность Beta", command=self.open_beta_readiness)
        ...
        tools_menu.add_command(label="Совместная работа", command=self.open_collaboration)
        tools_menu.add_command(label="Объединение проектов", command=self.open_project_merge)
        tools_menu.add_command(label="Разрешение конфликтов", command=self.open_conflict_resolution)
        tools_menu.add_command(label="Просмотр истории", command=self.open_history_browser)
        tools_menu.add_command(label="Автоматизация процессов", command=self.open_workflow_automation)
        tools_menu.add_command(label="Автономный обмен изменениями", command=self.open_offline_change_exchange)
        ...
        analysis_menu.add_command(label="Центр анализа", command=self.open_intelligence_center)
        analysis_menu.add_command(label="Анализ источников", command=self.open_source_analysis_center)
    ...
    def _add_selected_recent_to_favorites(self):
        person_id = self._selected_recent_person_id()
        if person_id is None:
            messagebox.showwarning("Недавние люди", "Сначала выберите человека.")
            return
        added = self._favorite_service().add(person_id)
        status = getattr(self, "status_label", None)
        if status is not None:
            status.config(
                text="Человек добавлен в избранное."
                if added
