...
def test_reports_are_deterministic_and_missing_audit_identity_correlation_is_a_blocker(tmp_path):
    _, service = validator(tmp_path); report = service.validate(); first = [path.read_bytes() for path in service.export_all(report)]
    assert report.recommendation == "NOT READY FOR 2.1.0-BETA1"
    assert next(check for check in report.checks if check.check_id == "audit.correlation").status == BLOCKED
